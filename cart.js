...
async function fetchCartFromBackend() {
    ...
    try {
        ...
        const transformedCart = backendCart.map(item => {
            ...
            const sizeIndex = item.productSizes?.indexOf(item.selectedSize) ?? 0;

            let sellingPrice = 0;
            let mrpPrice = 0;

            if (Array.isArray(item.price) && item.price.length > sizeIndex) {
                sellingPrice = Number(item.price[sizeIndex]);
            } else if (Array.isArray(item.price) && item.price.length > 0) {
                sellingPrice = Number(item.price[0]); // fallback to first
            }

            // For MRP: use second price if available, else calculate
            if (Array.isArray(item.price) && item.price.length > sizeIndex + 1) {
                mrpPrice = Number(item.price[sizeIndex + 1]);
            } else if (Array.isArray(item.originalPrice) && item.originalPrice.length > sizeIndex) {
                mrpPrice = Number(item.originalPrice[sizeIndex]);
            } else {
                mrpPrice = sellingPrice * 1.3; // fallback
            }
